...
function SketchLine(_numberOfVertices, _easeFactor, _speedFactor, _colors) {
    ...
    for (var i = 0; i < this.numberOfVertices; i++) {
        this.curveVertices[i] = createVector(sensorX, sensorY);
...
SketchLine.prototype.update = function() {
    ...
    for (var i = 0; i < this.numberOfVertices; i++) {
        this.distances[i].x = (i === 0) ? sensorX - this.curveVertices[0].x : this.curveVertices[i - 1].x - this.curveVertices[i].x;
        this.distances[i].y = (i === 0) ? sensorY - this.curveVertices[0].y : this.curveVertices[i - 1].y - this.curveVertices[i].y;
